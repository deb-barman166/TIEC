...
import { LogOut, Calendar, Save, Terminal, CheckCircle2, Users, BookOpen, Bell, Plus, Trash2, Settings } from 'lucide-react';
    ...
    <div className="min-h-screen bg-bg-base flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 glass-panel border-r border-white/5 flex flex-col md:h-screen sticky top-0 z-20">
        <div className="p-6 border-b border-white/5 flex items-center justify-between md:justify-start gap-3">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center p-[1px]">
            <div className="w-full h-full bg-bg-surface rounded-lg flex items-center justify-center">
              <Terminal className="w-5 h-5 text-red-500" />
            </div>
          </div>
          <span className="font-heading font-bold text-lg text-white">Super Admin</span>
        </div>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        <header className="mb-10">
          <h1 className="text-3xl font-heading font-bold text-white mb-2">
            Welcome back, <span className="gradient-text from-red-500 to-orange-500">Super-Amide</span>
          </h1>
            ...
            <div className="glass-panel p-8 rounded-2xl border border-white/10">
              ...
              <form onSubmit={handleAddStudent} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                <div className="space-y-2">
                  ...
                  <input required type="text" value={newStudent.name} onChange={e => setNewStudent({...newStudent, name: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="Student Name" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-text-secondary uppercase">Email (@tiec.edu)</label>
                  <input required type="email" value={newStudent.email} onChange={e => setNewStudent({...newStudent, email: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="student@tiec.edu" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-text-secondary uppercase">Password</label>
                  <input required type="text" value={newStudent.password} onChange={e => setNewStudent({...newStudent, password: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="Password" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-text-secondary uppercase">Group</label>
                  <select value={newStudent.groupId} onChange={e => setNewStudent({...newStudent, groupId: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none appearance-none">
                    ...
                    {students.map(student => (
                      <tr key={student.id} className="border-b border-white/5 text-white text-sm">
                        ...
                        <td className="py-4 text-right">
                          <button onClick={() => handleDeleteStudent(student.id)} className="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="glass-panel p-8 rounded-2xl border border-white/10">
              ...
              <form onSubmit={handleAddGroup} className="flex flex-col md:flex-row gap-4 items-end">
                <div className="space-y-2 flex-1">
                  ...
                  <input required type="text" value={newGroup.name} onChange={e => setNewGroup({ ...newGroup, name: e.target.value })} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="e.g. Web Dev Batch B" />
                </div>
                <div className="space-y-2 flex-1">
                  <label className="text-xs text-text-secondary uppercase">Course</label>
                  <input required type="text" value={newGroup.course} onChange={e => setNewGroup({ ...newGroup, course: e.target.value })} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="e.g. Web Development" />
                </div>
              ...
              {groups.map(group => {
                ...
                return (
                  <div key={group.id} className="glass-panel p-6 rounded-2xl border border-white/10 flex flex-col justify-between">
                    ...
                    <button onClick={() => handleDeleteGroup(group.id)} className="flex items-center justify-center gap-2 w-full py-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors text-sm font-medium">
              ...
              <form onSubmit={handleAddNotice} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input required type="text" value={newNotice.title} onChange={e => setNewNotice({...newNotice, title: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none" placeholder="Title" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs text-text-secondary uppercase">Type</label>
                    <select value={newNotice.type} onChange={e => setNewNotice({...newNotice, type: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none appearance-none">
                ...
                <div className="space-y-2">
                  ...
                  <textarea required value={newNotice.content} onChange={e => setNewNotice({...newNotice, content: e.target.value})} className="w-full bg-bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-red-500 focus:outline-none min-h-[100px]" placeholder="Notice details..." />
                </div>
            ...
            <div className="space-y-4">
              {notices.map(notice => (
                <div key={notice.id} className="glass-panel p-6 rounded-2xl border border-white/10 flex justify-between items-start gap-4">
                  ...
                  <button onClick={() => handleDeleteNotice(notice.id)} className="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors shrink-0"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </motion.div>
            ...
            <form onSubmit={handleSaveDates} className="glass-panel p-8 rounded-2xl border border-white/5 shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input 
                  ...
                  className="w-full bg-bg-surface border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" 
                />
              </div>
